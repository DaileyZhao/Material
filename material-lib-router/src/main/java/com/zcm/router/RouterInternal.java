...
import com.zcm.router.exception.NotRouteException;
import com.zcm.router.rule.ActivityRule;
import com.zcm.router.rule.ReceiverRule;
import com.zcm.router.rule.Rule;
import com.zcm.router.rule.ServiceRule;
...
public class RouterInternal {
     ...
     * @return
     */
    final boolean isExistRouter(String pattern) {
        Rule<?, ?> rule = getRule(pattern);
        return rule != null && rule.isExistRule(pattern);
    }
}
